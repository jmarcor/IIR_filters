...
int main(int argc, char** argv) {
	...
	IIR_MD_t *filter = IIR_MD_create( n_coefs, n_signals, b, a );
    ...
    for ( int i=0; i<10; i++ ){
        IIR_signal_t *tmp = IIR_MD_add_input(filter, input);
        for ( int j=0; j<n_signals; j++ ){
            printf( "%.4" IIR_SIGNAL_FORMAT ", %.4" IIR_SIGNAL_FORMAT "\n" , tmp[j], IIR_MD_get_last_output(filter)[j] );
        }
        printf( "\n" );
    }    
    ...
    for ( int i=0; i<10; i++ ){
        IIR_MD_add_input(filter, input);
        for ( int j=0; j<n_signals; j++ ){
            printf( "%.4" IIR_SIGNAL_FORMAT ", " , IIR_MD_get_last_output(filter)[j] );
        }
        printf( "\n" );
    }
