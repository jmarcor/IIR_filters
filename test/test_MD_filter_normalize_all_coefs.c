...
//        }        
        ...
        IIR_signal_t *b_base = MD_filter->b;
        IIR_signal_t *a_base = MD_filter->a;
        for ( int j=0; j < N_SIGNALS; j++ ){
            ...
            // Set coefs without normalizing!
            memcpy( b_base, b_coefs, n_coefs*sizeof(IIR_signal_t) );
            memcpy( a_base, a_coefs, n_coefs*sizeof(IIR_signal_t) );
            b_base += n_coefs;
            a_base += n_coefs;
        }
        
        // Now use the function to normalize all coefs
        IIR_MD_normalize_all_coefs( MD_filter );
