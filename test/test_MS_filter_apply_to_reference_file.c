...
int main(int argc, char** argv) {
    ...
    if ( loaded_data ){
        ...
        IIR_MS_t *filter = IIR_MS_create( n_coefs, N_INPUT_SIGNALS, b_coefs, a_coefs );        
        ...
        for ( int i=0; i < n_inputs; i++ ){
            ...
            my_outputs[i] = (IIR_MS_add_input(filter, current_input))[0];
        }
        ...
        printf( "\nSUCCESS: IIR_MS: Binary reference file '%s' generated\n", out_fname );    
    ...
    printf( "\nSUCCESS: IIR_MS: Filter applied and output binary file created\n" );
    return EXIT_SUCCESS;
}
