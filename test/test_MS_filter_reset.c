...
int main(int argc, char** argv) {
    ...
    if ( loaded_data ){
        ...
        // Create two filters
        IIR_MS_t *filter1 = IIR_MS_create( n_coefs, N_SIGNALS, b_coefs, a_coefs );
        IIR_MS_t *filter2 = IIR_MS_create( n_coefs, N_SIGNALS, b_coefs, a_coefs );
        ...
        for ( int i=0; i < n_inputs; i++ ){
            ...
            IIR_MS_add_input(filter1, multi_signal_input);
        }
        
        // Reset
        IIR_MS_reset( filter1 );
        ...
        for ( int i=0; (!error) && (i < n_inputs); i++ ){
            ...
            IIR_MS_add_input(filter1, multi_signal_input);
            IIR_MS_add_input(filter2, multi_signal_input);
            for ( int j=0; j < N_SIGNALS; j++ ){
        ...
        IIR_MS_destroy(filter1);
        IIR_MS_destroy(filter2);
    ...
    if (error){
        printf( "\nERROR: IIR_MS: Filter reset does not work\n" );
        return EXIT_FAILURE;
    }

    printf( "\nSUCCESS: IIR_MS: Filter reset does work\n" );
    return EXIT_SUCCESS;
}
